...
ggplot(strain.df) + geom_line(aes(x = time, y = mean.norm.act)) + geom_errorbar(aes(x = time, ymin = mean.norm.act - sd.norm.act, ymax = mean.norm.act + sd.norm.act)) + geom_line(aes(x = time, y = median.norm.act, color = "red")) + facet_wrap( ~ strain) + labs(x = "Time (minutes)", y = "Normalized Activity") + ylim(0, 12) + presentation
...
well_time.df <- well.df %>% filter(time == timePoint2)
